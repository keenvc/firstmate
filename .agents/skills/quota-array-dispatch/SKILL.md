...
A candidate that will run on a non-default config seat carries a seat marker so it is not judged against the default account's row; the helper's header and `--help` own its exact syntax and what it does and does not claim.
