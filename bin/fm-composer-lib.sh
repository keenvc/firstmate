...
FM_DELIVERY_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working(\.\.\.|…)|Ctrl\+c:cancel|ctrl\+c to stop|esc[[:space:]]+to[[:space:]]+cancel|ESC: pause|esc twice to interrupt|^[[:space:]]*❭ Guide Devin while it works$'
...
# CLI 1.16.0).
FM_COMPOSER_IDLE_RE_DEFAULT='^Type a message\.\.\.$|^Ask anything(\.\.\.|…)|^Plan, search, build anything$|^Add a follow-up$|^What can I do for you\?$|^Type your message, @mention a file|^Ask Devin to build features, fix bugs, or work on your code$'
