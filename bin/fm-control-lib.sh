...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    muse|gemini|rovo|agy|devin|cline|openhands) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}
...
# through Herdr). cline cancels a running turn on a single Escape while its
# busy row reads `(esc to cancel)`; the turn stops and the composer returns to
# the `Ask anything...` placeholder with no prompt repollution (verified live,
# cline 3.0.62).
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|omp|kimi|cursor|gemini|muse|rovo|agy|devin|cline|openhands) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_arm_signal() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|muse|rovo|agy|cline|openhands) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_press_gap() {  # <harness>
  case "${1-}" in
    devin) printf '0.5' ;;
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|muse|rovo|agy|cline|openhands) printf '0.2' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|devin|cline|openhands) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|devin|cline|openhands) printf 'none' ;;
    *) return 1 ;;
  esac
}
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    # cline discovers its hook config files from the workspace's .cline/hooks
    # directory at session start, so the per-task wiring is a set of
    # worktree-resident executable files. Relaunch (including a harness switch)
    # removes each one; the shared clear path also prunes the emptied parents.
    cline)
