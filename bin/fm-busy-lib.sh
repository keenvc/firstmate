...
# the ONLY per-harness rendered-text sources that survive the redesign, because
# none of their structured lifecycles was credited-live-verified in the approved
# audit (Rovo's clean ACP stopReason lives outside the TUI path firstmate
# drives, see references/harness/rovo.md; agy 1.2.0 exposes no hook surface at
# all, see references/harness/agy.md; OpenHands CLI 1.16.0 exposes none
# firstmate can write, see references/harness/openhands.md); each is scoped to
...
# pinned at the fm-spawn seed keeps reading busy fm-spawn, unchanged. The
# public fm_busy_classify below wraps this with the quota override.
