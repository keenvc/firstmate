...
# entrypoint is invoked. While state/.afk-contract exists any green merge may
# proceed under away authority: the record's presence is the whole mechanical
# fact, and which merge the captain's away words meant is the supervision
# session's reading (bin/fm-branch-prompt.sh "Postures"). Neither posture
# releases a captain hold, and away authority lapses when the record is
# archived.
...
# record's presence and a captain hold say; this function owns what a merge run
# may do about it, so the answer the merge poll later tags its ledger row with is
# the same answer resolved here. An unreadable record or hold refuses rather than
# being skipped.
