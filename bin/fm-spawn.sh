...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  case "${POS[1]:-}" in
  '' | claude | codex | opencode | pi | pi-signed | grok | kimi | cursor | gemini | muse | rovo | omp | agy | devin | openhands)
    ARG3=${POS[1]:-}
    ;;
  *' '*)
...
# A home's worker account pin (config/claude-account) is the captain's own
# choice of Claude account and outranks a per-spawn seat: an explicit
# --claude-config-dir beside an active pin is two account authorities for one
# launch and is refused, and a seat recovered from a task's own record stays
# recorded but is not used while the pin is active, so removing the pin
# returns the lane to its seat.
if [ -n "$WORKER_ACCOUNT" ] && [ "$HARNESS" = claude ] && [ -n "$CLAUDE_SEAT_DIR" ]; then
  if [ "$RELAUNCH" -eq 0 ] && [ -n "$CLAUDE_SEAT_ARG" ]; then
    echo "error: --claude-config-dir conflicts with this home's Claude worker account pin (config/claude-account); the pin is the captain's account choice, so remove one of them" >&2
    exit 1
  fi
  echo "warning: this home's Claude worker account pin outranks the recorded Claude config directory '$CLAUDE_SEAT_DIR'; the launch uses the pinned account and the record is kept" >&2
  CLAUDE_SEAT_DIR=
fi
...
model_flag_for_harness() {
  ...
  [ -n "$model" ] && [ "$model" != default ] || return 0
  case "$harness" in
  claude | codex | opencode | pi | pi-signed | grok | kimi | cursor | gemini | muse | rovo | omp | agy | devin | cline)
