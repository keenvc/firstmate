...
# read) whatever the away-posture record says, and
...
fm_merge_authority_resolve() {  # <home> <state> <meta> <task-id>
  ...
  # posture is even consulted, so a held task can never resolve to away
  # authority and a recorded hold cannot disagree with the merge path.
