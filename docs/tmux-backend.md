...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, AGY, and cline process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
cline is anchored to the exact `.cline` native-binary identity, with a node-wrapper fallback on the anchored path fragments `/bin/cline` and `@cline/cli`, so a script merely containing "cline" is not accepted.
